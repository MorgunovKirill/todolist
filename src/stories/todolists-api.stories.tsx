...
export const DeleteTodolist = () => {
    const [state, setState] = useState<any>(null)
    useEffect(() => {
        const todoId = '59e9d100-e356-4d1e-a5de-0747f98df832'
...
export const UpdateTodolistTitle = () => {
    ...
    useEffect(() => {
        ...
        const title = 'React';
        api.updateTodolist(todoId, title).then((res) => {
            setState(res.data);
        })
    }, [])
...
export const UpdateTask = () => {
    ...
    const updateTaskHandler = () => {
        api.updateTask(todolistId, taskId, {
            description,
            title: newTitle,
            status,
            priority,
    ...
    return <div>
        <input type="text" placeholder={'todolistId'} value={todolistId}
               onChange={(e) => setTodolistId(e.currentTarget.value)}/>
        <input type="text" placeholder={'taskId'} value={taskId} onChange={(e) => setTaskId(e.currentTarget.value)}/>
        <input type="text" placeholder={'New title'} value={newTitle} onChange={(e) => setNewTitle(e.currentTarget.value)}/>
        ...
        <input type="text" placeholder={'priority'} value={priority} onChange={(e) => setPriority(+e.currentTarget.value)}/>
